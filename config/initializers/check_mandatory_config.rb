...
if Conf.api.tls_fingerprint.blank?
  fatal "Error: 'tls_fingerprint' is empty, can't verify connection to API without it (in config/schleuder-web.yml)."
end

if Conf.api_use_tls? && Conf.api_key.blank?
  fatal "Error: 'api_key' is empty but required if 'use_tls' is true (in config/schleuder-web.yml)."
end
